/* /picjeju_next/admin/assets/js/post-add.js
 * 새 포스트 작성 — /picjeju_next/admin/content/post/add.php
 */

document.addEventListener('DOMContentLoaded', () => {
  
    const $ = (s, r = document) => r.querySelector(s);
    const KEY = 'ADMIN_POSTS';
     ...
     * 에디터 초기화 (Toast UI)
     * ------------------------------ */
    const editorEl = document.querySelector('#postEditor');
    const fallback = document.querySelector('#postEditorFallback');
    ...
    try {
      ...
      console.warn('에디터 로딩 실패 → 폴백 textarea 표시');
    ...
    $('#postForm')?.addEventListener('submit', (e) => {
      ...
      const dto = {
        ...
        desc:
          ...
          $('#postEditorFallback textarea')?.value.trim() ||
          '',
      ...
      showToast('포스트가 저장되었습니다 ✅', 'primary');
      setTimeout(() => {
        location.href = (window.ADMIN_BASE_PATH || '/picjeju_next/admin') + '/content/post';
      }, 1800);
    });
